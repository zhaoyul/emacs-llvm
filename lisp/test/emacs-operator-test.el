...
(ert-deftest emacs-operator-alpha18-capabilities-report-symbol-source ()
  (emacs-operator-test-with-buffer
   (let ((source (make-temp-file "emacs-operator-symbol-source-" nil ".el")))
     (unwind-protect
         (progn
           (with-temp-file source
             (insert "(defun emacs-operator-alpha18-source-command () (interactive) t)\n"))
           (load source nil t)
           (let* ((target `(("buffer_id" . ,(emacs-operator-buffer-id))))
                  (result (emacs-operator-capabilities-query
                           `(("target" . ,target)
                             ("operation" . "symbol_source")
                             ("command" . "emacs-operator-alpha18-source-command")))))
             (should (equal (emacs-operator--get result "symbol")
                            "emacs-operator-alpha18-source-command"))
             (should (emacs-operator--truthy-json-p
                      (emacs-operator--get result "interactive")))
             (should (equal (file-truename (emacs-operator--get result "source"))
                            (file-truename source)))))
       (ignore-errors (fmakunbound 'emacs-operator-alpha18-source-command))
